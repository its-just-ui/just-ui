...
const Switch = forwardRef<HTMLInputElement, SwitchProps>(
  (
    ...
    const trackStyles: React.CSSProperties = {
      ...
      backgroundColor:
        isHovered && hoverTrackBackgroundColor
          ? hoverTrackBackgroundColor
      ...
      borderColor:
        isHovered && hoverBorderColor
          ? hoverBorderColor
          : (defaultStyles.track as React.CSSProperties).borderColor || 'transparent',
    ...
    const thumbStyles: React.CSSProperties = {
      ...
      backgroundColor:
        isHovered && hoverThumbBackgroundColor
          ? hoverThumbBackgroundColor
      ...
      borderColor:
        checked && thumbBorderColorChecked
          ? thumbBorderColorChecked
          : thumbBorderColor || 'transparent',
      borderStyle: thumbBorderStyle || 'solid',
      borderRadius: thumbBorderRadius || '50%',
      boxShadow:
        checked && thumbBoxShadowChecked ? thumbBoxShadowChecked : defaultStyles.thumb.boxShadow,
    ...
    const focusStyles: React.CSSProperties = isFocused
      ? {
          ...
          boxShadow:
            focusBoxShadow ||
            `0 0 0 ${focusRingWidth || '2px'} ${focusRingColor || statusColors.trackChecked}`,
        }
      : {}
    ...
    const renderThumbContent = () => {
      ...
      if (checked && thumbIconChecked) {
        return (
          <span style={{ color: thumbIconColorChecked || 'currentColor' }}>{thumbIconChecked}</span>
        )
      }

      if (!checked && thumbIcon) {
        return <span style={{ color: thumbIconColor || 'currentColor' }}>{thumbIcon}</span>
      }
    ...
    return (
      ...
      >
        <div className={cn('inline-flex flex-col', containerClassName)} style={containerStyles}>
          <label
            className={cn(
          ...
          >
            ...
            <div className="relative inline-flex">
              <input
                ...
                aria-describedby={
                  errorMessage ? 'switch-error' : helperText ? 'switch-helper' : undefined
                }
                {...props}
              />
              ...
              >
                <div style={thumbStyles}>{renderThumbContent()}</div>
              </div>
            </div>
